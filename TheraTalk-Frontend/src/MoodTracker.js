          ...
          <div>
            <Calendar
              ...
              tileContent={({ date, view }) => {
                const moodForDate = moodHistory.find(entry => new Date(entry.timestamp).toLocaleDateString() === date.toLocaleDateString());
                return moodForDate ? <p>{moodForDate.mood}</p> : null;
              }}
            />
            <div>
              {moodHistory
                .filter(entry => new Date(entry.timestamp).toLocaleDateString() === selectedDate.toLocaleDateString())
                .map((entry, index) => (
                  <p key={index}>{entry.mood} - {new Date(entry.timestamp).toLocaleString()}</p>
              ))}
            </div>
          </div>
        )}
