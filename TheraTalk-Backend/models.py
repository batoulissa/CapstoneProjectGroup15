...
def get_next_patient_id():
    # Query max patient_id or default to 100000 (so next is 100001)
    max_id = db.session.query(func.max(Patient.patient_id)).scalar()
    if not max_id or max_id < 100000:
        return 100001
    return max_id + 1

def get_next_therapist_id():
    # Query max therapist_id or default to 200000 (so next is 200001)
    max_id = db.session.query(func.max(Therapist.therapist_id)).scalar()
    if not max_id or max_id < 200000:
        return 200001
    return max_id + 1
...
class Therapist(db.Model):
    ...
    therapist_id = db.Column(db.Integer, primary_key=True, autoincrement=False)
    ...
    def __init__(self, name, username, pronouns, experience_years, languages, focus_areas, password, proof_of_education, status="pending", availability=None):
        self.therapist_id = get_next_therapist_id() 
        self.name = name
        self.username = username
...
class Patient(db.Model):
    ...
    patient_id = db.Column(db.Integer, primary_key=True, autoincrement=False)
    ...
    def __init__(self, name, username, birthdate, pronouns, medical_history, therapy_need, languages, password, status, patient_id_card):
        self.patient_id = get_next_patient_id()
        self.name = name
        self.username = username
